...
// Variável para armazenar o total das Planos
let totalPlano = 0;
...
const Transaction = {
    add(transaction, monthIndex) {
        ...
        deleteButton.onclick = function() {
            ...
            totalPlano -= transaction.amount;
            updatePlanoDisplay(); // Atualiza o valor total dos Planos
            removeTransactionFromLocalStorage(transaction);
            // Atualiza o valor total das Planos após deletar a transação
            totalPlanoDisplay.textContent = `R$ ${totalPlano.toFixed(2)}`;
            filteredPlanoDisplay.textContent = `R$ ${getTotalFilteredPlano().toFixed(2)}`;
        };
        deleteCell.appendChild(deleteButton);

        // Atualiza o valor total das Planos ao adicionar uma nova transação
        totalPlano += transaction.amount;
...
function saveTransactionToLocalStorage(transaction) {
    let transactions = JSON.parse(localStorage.getItem('plano_transactions')) || [];
    ...
    if (existingTransactionIndex === -1) {
        ...
        localStorage.setItem('plano_transactions', JSON.stringify(transactions));
    }
}
...
// Função para atualizar a exibição do total de Planos nos cartões
function updatePlanoDisplay() {
    console.log("Atualizando exibição dos planos");

    // Obtém os elementos de exibição dos valores totais e filtrados
    const totalPlanoDisplay = document.getElementById('totalPlanoDisplay');
    const filteredPlanoDisplay = document.getElementById('filteredPlanoDisplay');

    // Calcula o valor total de todas as transações
    const totalPlano = getTotalPlano();

    // Atualiza o valor no cartão de Planos totais
    totalPlanoDisplay.textContent = `R$ ${totalPlano.toFixed(2)}`;

    // Se houver transações filtradas, calcule o valor total com base nelas
    if (hasFilteredTransactions()) {
        const totalFilteredPlano = getTotalFilteredPlano();

        // Atualiza o valor no cartão de Planos filtradas
        filteredPlanoDisplay.textContent = `R$ ${totalFilteredPlano.toFixed(2)}`;
    } else {
        // Caso contrário, exiba o valor total de todas as transações nos dois cartões
        filteredPlanoDisplay.textContent = `R$ ${totalPlano.toFixed(2)}`;
    }
}

function getTotalPlano() {
    const transactions = JSON.parse(localStorage.getItem('plano_transactions')) || [];
...
// Função para calcular o valor total das planos filtradas
function getTotalFilteredPlano() {
    const transactions = JSON.parse(localStorage.getItem('plano_transactions')) || [];
...
const Form = {
    submit(event) {
        ...
        try {
            ...
            // Atualiza a exibição das Planos após adicionar uma transação
            updatePlanoDisplay(); 
        } catch (error) {
...
// Event listener para abrir o modal ao clicar no botão "+ Adicionar Plano"
...
document.getElementById('filter-form').addEventListener('submit', function(event) {
    ...
    // Atualiza a exibição das planos filtradas após filtrar as transações
    updateFilteredPlanoDisplay();

    // Fecha o modal de filtro após a filtragem
    FilterModal.close();
});

// Função para atualizar a exibição do total de Planos filtradas nos cartões
function updateFilteredPlanoDisplay() {
    console.log("Atualizando exibição de planos filtradas");
